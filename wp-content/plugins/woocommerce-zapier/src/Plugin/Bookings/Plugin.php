...
class Plugin extends Base {
	...
	public function initialise() {
		...
		add_action(
			'woocommerce_before_booking_object_save',
			function ( $booking ) {
				if ( ! $booking->get_id() ) {
					return;
				}
				...
				/**
				 * Execute the Zapier Integration handler for woocommerce_before_booking_object_save.
				 *
				 * @param int $booking_id Booking ID.
				 *
				 * @since 2.2.0
				 ...
				 * @internal
				 */
				do_action( 'wc_zapier_woocommerce_booking_updated', $booking->get_id() );
			}
		);
