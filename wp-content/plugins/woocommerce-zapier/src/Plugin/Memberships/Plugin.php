...
class Plugin extends Base {
	 ...
	 */
	const MINIMUM_SUPPORTED_VERSION = '1.26.0';
	...
	/**
	 * Remove Memberships endpoints that are not required by Zapier Integration, including:
	 *
	 * - /wc-zapier/v1/memberships/members/directory
	 *
	 * @since 2.17.0
	 *
	...
	public function filter_rest_endpoints( $endpoints ) {
		foreach ( $endpoints as $route => $endpoint ) {
			if ( 0 === strpos( $route, sprintf( '/%s/memberships/members/directory', API::REST_NAMESPACE ) ) ) {
				// Added in WooCommerce Memberships v1.27.3.
