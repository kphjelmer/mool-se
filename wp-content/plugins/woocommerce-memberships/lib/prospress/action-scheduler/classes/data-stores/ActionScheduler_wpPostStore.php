...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	public function query_actions( $query = array(), $query_type = 'select' ) {
		...
		return ( 'count' === $query_type ) ? $wpdb->get_var( $sql ) : $wpdb->get_col( $sql ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
	...
	public function get_claim_count(){
		...
		$sql = $wpdb->prepare( $sql, array( self::POST_TYPE ) ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function find_actions_by_claim_id( $claim_id ) {
		...
		$sql = $wpdb->prepare( $sql, array( self::POST_TYPE, $claim_id ) ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function release_claim( ActionScheduler_ActionClaim $claim ) {
		...
		$sql = $wpdb->prepare( $sql, array( $claim->get_id() ) ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function unclaim_action( $action_id ) {
		...
		$sql = $wpdb->prepare( $sql, $action_id, self::POST_TYPE ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$result = $wpdb->query($sql); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		if ( $result === false ) {
	...
	public function mark_failure( $action_id ) {
		...
		$sql = $wpdb->prepare( $sql, self::STATUS_FAILED, $action_id, self::POST_TYPE ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$result = $wpdb->query($sql); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		if ( $result === false ) {
			/* translators: %s: action ID */
	...
	private function get_post_column( $action_id, $column_name ) {
		/** @var \wpdb $wpdb */
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT {$column_name} FROM {$wpdb->posts} WHERE ID=%d AND post_type=%s", $action_id, self::POST_TYPE ) ); // @phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
	...
	public function log_execution( $action_id ) {
		...
		$sql = $wpdb->prepare( $sql, self::STATUS_RUNNING, current_time('mysql', true), current_time('mysql'), $action_id, self::POST_TYPE ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		if ( strlen( json_encode( $action->get_args() ) ) > self::$max_index_length ) {
			_doing_it_wrong( 'ActionScheduler_Action::$args', sprintf( 'To ensure the action args column can be indexed, action args should not be more than %d characters when encoded as JSON. Support for strings longer than this will be removed in a future version.', self::$max_index_length ), '2.1.0' ); // @phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
