...
abstract class ActionScheduler_Abstract_ListTable extends WP_List_Table {
	...
	protected function process_bulk_action() {
		...
		if ( array_key_exists( $action, $this->bulk_actions ) && is_callable( array( $this, $method ) ) && ! empty( $_GET['ID'] ) && is_array( $_GET['ID'] ) ) {
			$sanitized_ids = array_map( 'sanitize_text_field', $_GET['ID'] );
			$ids_sql = '(' . implode( ',', array_fill( 0, count( $sanitized_ids ), '%s' ) ) . ')';
			$this->$method( $sanitized_ids, $wpdb->prepare( $ids_sql, $sanitized_ids ) ); // @phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}

		wp_redirect( remove_query_arg( // @phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
			array( '_wp_http_referer', '_wpnonce', 'ID', 'action', 'action2' ),
			wp_unslash( sanitize_text_field( $_SERVER['REQUEST_URI'] ) )
		) );
		exit;
	}
	...
	protected function bulk_delete( array $ids, $ids_sql ) {
		...
		$wpdb->query( "DELETE FROM {$this->table_name} WHERE {$this->ID} IN $ids_sql" ); // @phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	}
	...
	protected function get_request_order() {
		...
		if ( ! empty( $_GET['order'] ) && 'desc' === strtolower( sanitize_text_field( $_GET['order'] ) ) ) {
			$order = 'DESC';
		} else {
			$order = 'ASC';
		}
	...
	protected function get_items_query_search() {
		...
		foreach ( $this->search_by as $column ) {
			$filter[] = '`' . $column . '` like "%' . $wpdb->esc_like( sanitize_text_field( $_GET['s'] ) ) . '%"';
		}
	...
	protected function get_items_query_filters() {
		...
		foreach ( $this->filter_by as $column => $options ) {
			if ( empty( $_GET['filter_by'][ $column ] ) || empty( $options[ sanitize_text_field( $_GET['filter_by'][ $column ] ) ] ) ) {
				continue;
			}

			$filter[] = $wpdb->prepare( "`$column` = %s", sanitize_text_field( $_GET['filter_by'][ $column ] ) ); // @phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['_wp_http_referer'] ) ) {
			...
			wp_redirect( remove_query_arg( array( '_wp_http_referer', '_wpnonce' ), wp_unslash( sanitize_text_field( $_SERVER['REQUEST_URI'] ) ) ) ); // @phpcs:ignore WordPress.Security.SafeRedirect.wp_redirect_wp_redirect
			exit;
		}
	...
	public function extra_tablenav( $which ) {
		...
		foreach ( $this->filter_by as $id => $options ) {
			$default = ! empty( $_GET['filter_by'][ $id ] ) ? sanitize_text_field( $_GET['filter_by'][ $id ] ) : '';
	...
	protected function process_row_actions() {
		...
		if ( sanitize_text_field( $_REQUEST['nonce'] ) === wp_create_nonce( sanitize_text_field( $_REQUEST[ 'row_action' ] ) . '::' . sanitize_text_field( $_REQUEST[ 'row_id' ] ) ) && method_exists( $this, $method ) ) {
			$this->$method( sanitize_text_field( $_REQUEST['row_id'] ) );
		}
